...
The two files above handle this task. The nutrition cleaning file was my original file that only combined the nutritional data since that was at the time all I was concerned about. It is extremely inefficient and at some point I hope to remove it entirely.
...
Overall the base linear model only explained 13.3% and the stepwise model which only contained 10 variables explained 12.1%. I will need to go and access model fit and its parameters using a QQplot
...
I've spoken a bit on some of what I would like to explore above. Other aspects I would like to try are different clustering methods, splines, and possibly exploring more computationally intensive methods in python.
